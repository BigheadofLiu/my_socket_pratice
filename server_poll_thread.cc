#include<iostream>
#include<cstring>
#include<stdlib.h>
#include<arpa/inet.h>
#include<sys/socket.h>
#include<unistd.h> 
#include<poll.h> //using poll
#include<vector> //using vector
#include<thread> //using thread
#include<memory> 
#include<mutex>


std::vector<pollfd> fds;  //使用 vector替换fd_set  数据不支持动态扩展：pollfd fds[1024];
std::mutex g_mutex;

void comm_thread(int fd){
        char buffer[1024] {0}; //读写缓冲区
        bool need_close =false;  //判断 fd 是需要放回还是直接关闭
        int recv_ret=recv(fd,buffer,sizeof(buffer),0);
        if(recv_ret>0){
            std::cout<<"收到客户端（fd="<<fd<<")发来的消息："<<buffer<<std::endl;
            std::string msg="服务器确认收到消息:";
            msg+=buffer;
            int send_ret=send(fd,msg.c_str(),msg.length(),0);
            if(send_ret==-1){
                perror("send");
                need_close=true;
            }
        }else if(recv_ret==0){
            std::cout<<"客户端（fd="<<fd<<")已断开连接"<<std::endl;
            need_close=true;
        }else{
            /*-1*/
            perror("recv");
            need_close=true;
        }
        if(need_close){
            close(fd);
        }else{
            //其他情况需要放回 主线程fds
            std::lock_guard<std::mutex> locker(g_mutex);
            pollfd thread_fd;
            thread_fd.fd=fd;
            thread_fd.events=POLLIN;
            thread_fd.revents=0;
            fds.push_back(thread_fd);
        }
}

int main(){
        //1.创建用于监听的套接字
        int lfd=socket(AF_INET,SOCK_STREAM,0);
        if(lfd==-1){
            perror("socket");
        }

        //2.创建用于socket通信的地址
        sockaddr_in server_socket_addr{}; 
        server_socket_addr.sin_family=AF_INET;  //协议
        server_socket_addr.sin_port=htons(10000); //端口
        // inet_pton(AF_INET,"172.16.172.129",&server_socket_addr.sin_addr.s_addr); //ip
        server_socket_addr.sin_addr.s_addr=INADDR_ANY;
        
        //3.地址与套接字绑定  
        socklen_t server_len;
        server_len=sizeof(server_socket_addr);
        int opt = 1;
        setsockopt(lfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));  //预防端口一直占用
        int bd=bind(lfd,(sockaddr*)&server_socket_addr,server_len);
        if(bd==-1){
            perror("bind");
        }

        //4.开始监听
        int list=listen(lfd,128);  //128指的是最大连接数？
        if(list==-1){
            perror("listen");
        }

        

        pollfd pfd;
        pfd.fd=lfd;
        pfd.events=POLLIN;   //关注什么事件
        pfd.revents=0;       //实际发生了什么（内核修改）
        {
        std::lock_guard<std::mutex> locker(g_mutex);  //可加可不加
        fds.push_back(pfd);  //放入vector
        } //限制锁的范围

        while (1)
        {
            std::vector<pollfd> fds_temp;
            {   //拷贝一份
                std::lock_guard<std::mutex> locker(g_mutex);
                fds_temp=fds;
            }

           int ret=poll(fds_temp.data(),fds_temp.size(),-1);  

           if(ret==-1){
            perror("poll");
            break;
           }
           for (size_t i = 0; i < fds.size(); ++i)     //开始遍历 vector
           {
            if (fds_temp[i].revents & POLLIN)
            {
                if(fds_temp[i].fd==lfd){
                    //如果是用于监听的套接字
                    int cfd=accept(lfd,nullptr,nullptr);
                    if(cfd== -1){
                        perror("accept");
                    }
                    pollfd new_pfd;
                    new_pfd.fd=cfd;
                    new_pfd.events=POLLIN;
                    new_pfd.revents=0;
                    fds.push_back(new_pfd);  //放入容器

                }else{
                    //用于通信的套接字处理
                    bool should_handle=false;  //是否开启子线程标志
                    {
                        std::lock_guard<std::mutex> locker(g_mutex);
                        for(size_t j =0;j<fds.size();++j){
                            if(fds[j].fd==fds_temp[i].fd){
                                //如果 cfd 在 fds 中 取出来交给子线程处理
                                fds[j]=fds.back();
                                fds.pop_back();
                                i--;
                                should_handle=true;
                                break;
                            }
                        }
                    }
                    if(should_handle){
                        std::thread t1(comm_thread,fds_temp[i].fd);
                        t1.detach();
                    }
                }
            }
           }
        }
    }